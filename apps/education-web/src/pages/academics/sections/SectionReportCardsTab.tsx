...
export function SectionReportCardsTab({ sectionId }: { sectionId: string }) {
  ...
  const { data: reportCards, isLoading, refetch } = useQuery({
    queryKey: ["section-report-cards", sectionId],
    queryFn: () => api.reportCards.list({ sectionId }),
  });
    ...
    <div>
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold text-foreground">
          {t("reporting.sectionReportCards", "Section Report Cards")}
        </h3>
            ...
            {(!reportCards || reportCards.length === 0) && (
              <tr>
                <td colSpan={8} className="p-8 text-center text-sm text-muted-foreground">
                  ...
                  {t("reporting.noCardsForSection", "No report cards generated for this section yet.")}
                </td>
              </tr>
            )}
          </tbody>
