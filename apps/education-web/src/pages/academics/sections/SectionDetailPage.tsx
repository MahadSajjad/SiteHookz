...
import { CustomButton } from '@sitehookz/ui/src/components/CustomButton'; // Will verify package name
...
export default function SectionDetailPage() {
  ...
  const { data: subjectsData, isLoading: isSubjectsLoading } = useQuery({
    ...
    queryFn: () => api.subjectOfferings.list({ sectionId: id }),
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b">
                    <th className="p-4 font-semibold text-sm text-gray-600">Subject Name</th>
                    <th className="p-4 font-semibold text-sm text-gray-600">Code</th>
                    <th className="p-4 font-semibold text-sm text-gray-600">Teacher</th>
                    <th className="p-4 font-semibold text-sm text-gray-600">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {subjectsData?.items?.map((offering: any) => (
                    <tr key={offering.id} className="border-b hover:bg-gray-50">
                      <td className="p-4 text-sm">{offering.subject?.name || 'N/A'}</td>
                      <td className="p-4 text-sm">{offering.subject?.code || 'N/A'}</td>
                      <td className="p-4 text-sm">{offering.teacher?.name || 'Unassigned'}</td>
                      <td className="p-4 text-sm">
                        <CustomButton variant="ghost" size="sm">Edit</CustomButton>
                      </td>
                    </tr>
                  ))}
                  {(!subjectsData?.items || subjectsData.items.length === 0) && (
                    <tr>
