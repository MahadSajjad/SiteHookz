...
async function bootstrap() {
  ...
  app.use(
    helmetImport.default ? helmetImport.default() : (helmetImport as any)(),
  );
  ...
  app.enableCors({
    origin: (
      ...
      const isAllowedLocal =
        process.env.NODE_ENV !== "production" && allowedLocal.includes(origin);
      const isAllowedProd =
        origin === `https://${baseDomain}` || origin.endsWith(`.${baseDomain}`);
