...
export class MembershipsService {
  ...
  async getById(organizationId: string, membershipId: string) {
    ...
    if (!membership) throw new BusinessException('MEMBERSHIP_NOT_FOUND', 404, 'Membership not found');
    return membership;
  }

  async suspend(organizationId: string, membershipId: string, currentUserMembershipId: string) {
    if (membershipId === currentUserMembershipId) {
      throw new BusinessException('MEMBERSHIP_CANNOT_SUSPEND_SELF', 400, 'Cannot suspend your own membership');
    }
