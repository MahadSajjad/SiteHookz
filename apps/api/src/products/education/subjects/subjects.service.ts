...
export class SubjectsService {
  ...
  async create(tenant: TenantContext, data: CreateSubjectDto) {
    await this.authorizationService.assertPermission(tenant, P.EDUCATION.SUBJECTS.CREATE);
    try {
      return await this.repository.create(tenant, data);
    } catch (error: any) {
      if (error.code === 'P2002') {
        throw new BusinessException('SUBJECT_CODE_DUPLICATE', 'A subject with this code already exists.');
      }
      throw error;
    }
  }
  ...
  async findAll(tenant: TenantContext) {
    await this.authorizationService.assertPermission(tenant, P.EDUCATION.SUBJECTS.READ);
  ...
  async update(tenant: TenantContext, id: string, data: UpdateSubjectDto) {
    await this.authorizationService.assertPermission(tenant, P.EDUCATION.SUBJECTS.UPDATE);
  ...
  async archive(tenant: TenantContext, id: string) {
    await this.authorizationService.assertPermission(tenant, P.EDUCATION.SUBJECTS.ARCHIVE);
    await this.findById(tenant, id);
