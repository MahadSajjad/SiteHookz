...
export class TeachingAssignmentsService {
  ...
  async assign(tenant: TenantContext, data: AssignTeacherDto) {
    ...
    await this.authorizationService.assertPermission(tenant, P.EDUCATION.TEACHING_ASSIGNMENTS.CREATE, branchId);

    // Check active staff branch assignment
    const activeStaffAssignment = await this.prisma.staffBranchAssignment.findFirst({
      where: {
    ...
    const existing = await this.repository.findActiveAssignment(tenant, data.subjectOfferingId, data.staffMemberId);
    if (existing) {
      throw new BusinessException(
  ...
  async findBySubjectOfferingId(tenant: TenantContext, subjectOfferingId: string) {
    ...
    await this.authorizationService.assertPermission(tenant, P.EDUCATION.TEACHING_ASSIGNMENTS.READ, branchId);
  ...
  async findByStaffMemberId(tenant: TenantContext, staffMemberId: string) {
    ...
    const assignments = await this.repository.findByStaffMemberId(tenant, staffMemberId);
    ...
    for (const assignment of assignments) {
      const branchId = assignment.subjectOffering.schoolOffering 
        ? assignment.subjectOffering.schoolOffering.section.classLevel.course.branchId
        : assignment.subjectOffering.tuitionOffering!.batch.course.branchId;
  ...
  async endAssignment(tenant: TenantContext, id: string, data: EndTeachingAssignmentDto) {
    ...
    const branchId = await this.getBranchIdForOffering(assignment.subjectOfferingId);
    await this.authorizationService.assertPermission(tenant, P.EDUCATION.TEACHING_ASSIGNMENTS.UPDATE, branchId);
