...
import { CreateFeePlanDto, UpdateFeePlanDto, FeePlanListResponse } from '@sitehookz/education';
...
import { CreateFeePlanDtoSchema, UpdateFeePlanDtoSchema } from '@sitehookz/education';
...
export class FeePlansController {
  ...
  async create(
    ...
    @Body(new ZodValidationPipe(CreateFeePlanDtoSchema)) createDto: CreateFeePlanDto,
  ) {
    const result = await this.feePlansService.create(tenant.organizationId, createDto);
    return { data: result };
  }

  @Get()
  @RequirePermission('education.fee_plans.read')
  async findAll(@CurrentTenant() tenant: TenantContext): Promise<FeePlanListResponse> {
  ...
  async findOne(
    ...
    const result = await this.feePlansService.findOne(tenant.organizationId, id);
    return { data: result };
  }
  ...
  async update(
    @CurrentTenant() tenant: TenantContext,
    @Param('id', ParseUUIDPipe) id: string,
    @Body(new ZodValidationPipe(UpdateFeePlanDtoSchema)) updateDto: UpdateFeePlanDto,
  ) {
    const result = await this.feePlansService.update(tenant.organizationId, id, updateDto);
    return { data: result };
  }
  ...
  async activate(
    ...
    const result = await this.feePlansService.activate(tenant.organizationId, id);
    return { data: result };
  }
