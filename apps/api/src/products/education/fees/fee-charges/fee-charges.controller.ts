...
import { GenerateFeeChargesDtoSchema, VoidFeeChargeDtoSchema } from '@sitehookz/education';
...
export class FeeChargesController {
  ...
  async generate(
    ...
    @Body(new ZodValidationPipe(GenerateFeeChargesDtoSchema)) generateDto: GenerateFeeChargesDto,
  ) {
    const result = await this.feeChargesService.generate(tenant.organizationId, assignmentId, generateDto);
    return { data: result };
  }
}
...
export class FeeChargesVoidController {
  ...
  async voidCharge(
    ...
    @Body(new ZodValidationPipe(VoidFeeChargeDtoSchema)) voidDto: VoidFeeChargeDto,
  ) {
    const result = await this.feeChargesService.voidCharge(tenant.organizationId, tenant.membershipId, id, voidDto);
    return { data: result };
  }
}
