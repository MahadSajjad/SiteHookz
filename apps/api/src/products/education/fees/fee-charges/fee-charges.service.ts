...
import { GenerateFeeChargesDto, VoidFeeChargeDto, FeeCharge, FeeFrequency } from '@sitehookz/education';
...
export class FeeChargesService {
  ...
  async generate(organizationId: string, assignmentId: string, data: GenerateFeeChargesDto): Promise<FeeCharge[]> {
    return await this.prisma.$transaction(async (tx) => {
      ...
      if (assignment.endedAt) {
        throw new BadRequestException('Cannot generate charges for an ended assignment');
      }

      const monthlyItems = assignment.feePlan.items.filter(item => item.frequency === FeeFrequency.MONTHLY);
      
      const createdCharges: any[] = [];
      
      const [yearStr, monthStr] = data.billingPeriodKey.split('-');
      const dueDate = new Date(parseInt(yearStr), parseInt(monthStr) - 1, assignment.feePlan.defaultDueDay || 5);
  ...
  async voidCharge(organizationId: string, membershipId: string, chargeId: string, data: VoidFeeChargeDto): Promise<FeeCharge> {
    return await this.prisma.$transaction(async (tx) => {
      ...
      if (charge.voidedAt) throw new BadRequestException('Charge is already voided');

      // Check for valid payments
      const totalPaid = charge.paymentAllocations.reduce((sum, alloc) => sum.plus(alloc.amount), new Decimal(0));
      if (totalPaid.greaterThan(0)) {
        throw new BadRequestException('Cannot void a charge that has valid payment allocations');
      }
  ...
  async calculateStatus(chargeId: string): Promise<{ status: string, amountPaid: string, amountOutstanding: string }> {
    ...
    const validAllocations = charge.paymentAllocations.filter(a => !a.payment.voidedAt);
    const totalPaid = validAllocations.reduce((sum, a) => sum.plus(a.amount), new Decimal(0));
