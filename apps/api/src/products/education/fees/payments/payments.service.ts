  ...
  async create(
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      const sequence = await tx.paymentReceiptSequence.upsert({
        where: {
          organizationId_branchId: { organizationId, branchId: data.branchId },
        },
      ...
      for (const alloc of data.allocations) {
        ...
        const [charge] = await tx.$queryRaw<
          any[]
        >`SELECT id, amount, "voidedAt" FROM "FeeCharge" WHERE id = ${alloc.feeChargeId}::uuid AND "organizationId" = ${organizationId}::uuid FOR UPDATE`;

        if (!charge)
          throw new NotFoundException(`Charge ${alloc.feeChargeId} not found`);
        if (charge.voidedAt)
          throw new BadRequestException(
            `Charge ${alloc.feeChargeId} is voided`,
          );
        ...
        const alreadyPaid = existingAllocs.reduce(
        ...
        if (allocAmount.greaterThan(outstanding)) {
          throw new BadRequestException(
            `Allocation ${allocAmount} exceeds outstanding ${outstanding} for charge ${alloc.feeChargeId}`,
          );
        }
      }
      ...
      if (!totalAllocated.equals(new Decimal(data.amount))) {
        throw new BadRequestException(
          `Total allocations ${totalAllocated} do not match payment amount ${data.amount}`,
        );
      }
