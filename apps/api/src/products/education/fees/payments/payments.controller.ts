...
import { CreatePaymentDtoSchema, VoidPaymentDtoSchema } from '@sitehookz/education';
...
export class PaymentsController {
  ...
  async create(
    ...
    @Body(new ZodValidationPipe(CreatePaymentDtoSchema)) createDto: CreatePaymentDto,
  ) {
    const result = await this.paymentsService.create(tenant.organizationId, tenant.membershipId, createDto);
    return { data: result };
  }
  ...
  async voidPayment(
    ...
    const result = await this.paymentsService.voidPayment(tenant.organizationId, tenant.membershipId, id, voidDto);
    return { data: result };
  }
}
