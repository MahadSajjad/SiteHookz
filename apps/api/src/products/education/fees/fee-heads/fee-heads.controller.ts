...
import {
  CreateFeeHeadDtoSchema,
  UpdateFeeHeadDtoSchema,
} from "@sitehookz/education";
...
export class FeeHeadsController {
  ...
  async create(
    ...
    @Body(new ZodValidationPipe(CreateFeeHeadDtoSchema))
    createDto: CreateFeeHeadDto,
  ) {
    const result = await this.feeHeadsService.create(
  ...
  async findOne(
    ...
    const result = await this.feeHeadsService.findOne(
      tenant.organizationId,
      id,
    );
    return { data: result };
  ...
  async update(
    @CurrentTenant() tenant: TenantContext,
    @Param("id", ParseUUIDPipe) id: string,
    @Body(new ZodValidationPipe(UpdateFeeHeadDtoSchema))
    updateDto: UpdateFeeHeadDto,
  ) {
    const result = await this.feeHeadsService.update(
      tenant.organizationId,
      id,
      updateDto,
    );
