...
import { CreateEnrollmentFeePlanAssignmentDto, EnrollmentFeePlanAssignmentListResponse } from '@sitehookz/education';
...
export class FeeAssignmentsController {
  ...
  async create(
    ...
    @Body(new ZodValidationPipe(CreateEnrollmentFeePlanAssignmentDtoSchema)) createDto: CreateEnrollmentFeePlanAssignmentDto,
  ) {
    const result = await this.feeAssignmentsService.create(tenant.organizationId, tenant.membershipId, createDto);
    return { data: result };
  }
  ...
  async findAll(
    ...
    const data = await this.feeAssignmentsService.findAll(tenant.organizationId, studentEnrollmentId);
